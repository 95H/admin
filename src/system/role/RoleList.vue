    ...
    <table-page-template>
        ...
        <el-table :data='roles' border stripe style='width: 100%' slot='table'>
            ...
            <el-table-column label='操作' width='250'>
                <template slot-scope='{row}' v-if='row.readonly == 0'>
        ...
        <permission-selector slot='dialog' ref='permissionSelector'></permission-selector>
    </table-page-template>
</template>
<script>
...
export default {
    ...
    methods: {
        query() {
            let criteria = { orders: [{ column: 'readonly', asc: false }, { column: 'name' }] }
            if (this.searchValue) {
        ...
        editAuth(row) {
            ...
            this.$refs['permissionSelector'].show(row.id)
        },
        deleteRole(row) {
            let opt = {
