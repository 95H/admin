    ...
    <el-dialog title='权限管理' :visible.sync='dialogVisible' top='5vh' width='60%'>
        <div class='box'>
            <div class='custom-tree-container'>
                <div class='block'>
                    <el-tree ref='tree' :data='data' node-key='id' default-expand-all show-checkbox :props='treeProps' :expand-on-click-node='false' :default-checked-keys='checkedBoxArr' @check='getCurrentKey'>
                        <span class='custom-tree-node' slot-scope='{ node, data }'>
                            <span>{{ node.label }}</span>
                            <span v-if='data.level == "second"'>
                                <el-checkbox v-for='(opt,index) in data.opts' :key='index' v-model='opt.checked' @change='changeCheckBox(node, data)'>{{opt.name}}</el-checkbox>
                            </span>
                        </span>
                    </el-tree>
                </div>
...
export default {
    ...
    methods: {
        ...
        init() {
            let allMenus = adminapi.getAllMenus()
            let arr = utils.initCheckboxList(allMenus, this.permissions)
            //获取默认选中
