...
export default {
    ...
    methods: {
        async open(data) {
            console.log('user editor open', JSON.stringify(data))
            let vm = this
            ...
            vm.form.id = data.id
            vm.form.ssoId = data.ssoId
            vm.form.username = data.username
            vm.form.remark = data.remark
            vm.form.roleIds = []

            let userResp = await userapi.query({ criteria: { ssoId: data.ssoId }, withColumns: ['role'] })
            console.log('user query resp ', JSON.stringify(userResp))
            if (userResp.content) {
                let user = userResp.content[0]
                vm.form.roleIds = user.roleIds || []
            }
        },
        save() {
            let param = { ssoId: this.form.ssoId, remark: this.form.remark, roleIds: this.form.roleIds }
