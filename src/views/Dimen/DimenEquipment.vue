        ...
        <table-page-template>
            <el-form :inline='true' slot='form' size='mini' onSubmit='return false'>
                <el-form-item>
                    ...
                    <span class='span1'>
                        有
                        <span class='span1-1'>{{count}}</span>条sop没有设定质保期
                    </span>
                    <el-checkbox v-model='checked' label='查看没有质保期的sop及对象' name='type' @change='search'></el-checkbox>
                </el-form-item>
            </el-form>
        </table-page-template>
        <div class='patrol-bottom'>
            <page-table :total='pageData.total' :page='pageData.page' :size='pageData.size' @pageChanged='pageChanged' v-loading='loading'>
                <el-table :data='pageData.tableData' border stripe slot='table'>
                    ...
                    <el-table-column prop label='质保期' width='300'>
                        <template slot-scope='{row}'>
                            <div v-if='hasPermission("fm:maintenanceDuration:update")'>
                                <el-input class='input-1' v-model='row.duration' @keyup.native='changeNumber(row)' @blur='changeInput(row)' />天
                            </div>
                            <div v-else>
                                <el-input
                                    disabled='true'
