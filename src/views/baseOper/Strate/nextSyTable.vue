<template>
    <el-table :data='sysArr' ref='multipleTable' stripe slot='table' height='300' width='100%' border @selection-change='handleSelectionChange'>
...
export default {
    data() {
        return {
            multipleSelection: this.sysArr,
            SystemCodetoClass
        }
    },
    props: ['sysArr', 'activeName'],
    updated() {
    ...
    methods: {
        checked() {
            for (let i = 0; i < this.sysArr.length; i++) {
                this.$refs.multipleTable.toggleRowSelection(this.sysArr[i], true)
            }
        },
        handleSelectionChange(val) {
            this.multipleSelection = val
            this.$emit('setTableSy', this.multipleSelection)
        }
    }
}
</script>
