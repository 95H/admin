<template>
  <div class="run-segment">
    <div class="ground" :style="`width:${width}px;height:${height}px`" v-if="list">
      <!-- 图 -->
      <section
      ...
      <i
        ...
        :key="'key' + index"
        :style="`transform:  translate(${getPosition(item.activityTime1, '240000').offset - 15}px) rotate(45deg)`"
        >{{ item.activityTime1.substr(0, 2) + ':' + item.activityTime1.substr(2, 2) }}</i
      >
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    partInfo(item) {
      ...
      if (info.type === 'startOnly') {
        res = this.getPosition(item[0].activityTime1, '240000');
      } else if (info.type === 'endOnly') {
        res = this.getPosition('000000', item[0].activityTime1);
      } else {
        res = this.getPosition(item[0].activityTime1, item[item.length - 1].activityTime1);
      }
